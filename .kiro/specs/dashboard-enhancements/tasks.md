...
- [x] 1. Extend LocalStorage_Manager with four new keys
  - [x] 1.1 Add `theme`, `name`, `timerDuration`, and `sortOrder` keys to the `KEYS` constant in `LocalStorage_Manager` inside `js/app.js`
...
- [x] 2. Add Theme_Manager module and inline no-flash script
  - [x] 2.1 Add the inline no-flash `<script>` block to `index.html` inside `<head>`, before the `<link rel="stylesheet">` tag
  ...
  - [x] 2.2 Add the `Theme_Manager` IIFE module in `js/app.js` (place it after `LocalStorage_Manager` and before `Greeting_Widget`)
...
- [x] 3. Add theme toggle button to index.html and light-theme CSS overrides to style.css
  - [x] 3.1 Add the `#theme-toggle` button to `index.html` immediately after `<body>` opens, before the `.dashboard` div
  ...
  - [x] 3.2 Add the light-theme CSS rule and all new component styles to `css/style.css`
...
- [x] 4. Add Name_Manager module and Name_Input HTML
  - [x] 4.1 Add the `Name_Manager` IIFE module in `js/app.js` (place after `Theme_Manager`)
  ...
  - [x] 4.3 Add `#name-form` HTML to `index.html` inside the greeting card, below the `.date-display` paragraph
    - Use the markup from the design: form with `id="name-form"` containing `id="name-input"` text input and a submit button
    - _Requirements: 2.1_

- [x] 5. Extend Greeting_Widget to support Display_Name
  - [x] 5.1 Add `formatGreeting(hour, name)` pure function inside `Greeting_Widget`
  ...
  - [x] 5.2 Add `updateGreeting()` method to `Greeting_Widget`'s public API
    ...
    - _Requirements: 2.3, 2.4_
  - [x] 5.3 Update `_tick()` inside `Greeting_Widget` to call `formatGreeting(now.getHours(), Name_Manager.getName())` instead of `getGreeting(now.getHours())` when setting `elGreeting.textContent`
    - _Requirements: 2.3, 2.4_
  - [x] 5.4 Bind the `#name-form` submit event inside `Greeting_Widget.init()` (or a new `Name_Manager.bindForm()` helper called from the entry point)
...
- [x] 6. Checkpoint — verify theme and name features
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Add Timer_Config module and Duration_Input HTML
  - [x] 7.1 Add the `Timer_Config` IIFE module in `js/app.js` (place after `Name_Manager`, before `Greeting_Widget`)
  ...
  - [x] 7.3 Add `#timer-duration` Duration_Input HTML to `index.html` inside the timer card, above the `#timer-display` element
...
- [x] 8. Extend Focus_Timer to use Timer_Config
  - [x] 8.1 Replace the hardcoded `INITIAL_SECONDS = 25 * 60` in `Focus_Timer` with a dynamic value sourced from `Timer_Config.getDuration() * 60`
  ...
  - [x] 8.2 Add `Focus_Timer.setInitialSeconds(seconds)` public method
    ...
    - _Requirements: 3.3_
  - [x] 8.3 Bind the `#timer-duration` input's `'change'` event inside `Focus_Timer.init()`
  ...
  - [x] 8.4 Update `Focus_Timer._render()` to toggle `_elDuration.disabled = state.running` so the duration input is disabled while the timer is running and re-enabled after stop/reset
...
- [x] 10. Extend Todo_Manager with duplicate detection
  - [x] 10.1 Add `_isDuplicate(description, excludeId)` internal helper inside `Todo_Manager`
  ...
  - [x] 10.3 Update `addTask(description)` in `Todo_Manager` to call `_isDuplicate(trimmed, null)` after the empty-check; return `{ type: 'duplicate' }` if a duplicate is found; otherwise continue with existing create logic
    - _Requirements: 4.1_
  - [x] 10.4 Update `updateTask(id, description)` in `Todo_Manager` to call `_isDuplicate(trimmed, id)` after the empty/not-found checks; return `{ type: 'duplicate' }` if a duplicate is found; otherwise continue with existing update logic
    - _Requirements: 4.3_

- [x] 11. Add Sort_Control HTML, extend Todo_List with sort and error display
  - [x] 11.1 Add `#todo-sort` Sort_Control HTML to `index.html` inside the todo card, between `.section-title` and `#todo-add-form`
  ...
  - [x] 11.2 Add `#todo-add-error` error placeholder HTML to `index.html` immediately after the `#todo-add-form` closing tag
    - Use the markup from the design: `<p id="todo-add-error" class="field-error" role="alert" aria-live="polite" hidden></p>`
    - _Requirements: 4.2_
  - [x] 11.3 Add `_sortTasks(tasks, order)` pure helper function inside `Todo_List`
...
- [x] 12. Add duplicate error display to Todo_List
  - [x] 12.1 Implement `_showAddError(message)` and `_clearAddError()` inside `Todo_List`
    ...
    - _Requirements: 4.2_
  - [x] 12.2 Update the `#todo-add-form` submit handler inside `Todo_List.init()` to handle the `{ type: 'duplicate' }` return value
  ...
  - [x] 12.3 Update `_exitEditMode(id, save, newValue)` inside `Todo_List` to handle the `{ type: 'duplicate' }` return from `updateTask`
...
- [x] 13. Wire new modules into the entry point
  - [x] 13.1 Update the `DOMContentLoaded` entry point in `js/app.js` to initialize all new modules in the correct order
